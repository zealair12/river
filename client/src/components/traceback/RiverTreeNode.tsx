...
export interface RiverTreeNodeData {
  label: string;
  timestamp?: string;
  isActive: boolean;
  childCount: number;
...
function RiverTreeNodeComponent({ data }: NodeProps) {
  const { label, timestamp, isActive, childCount, isOnActivePath } = data as unknown as RiverTreeNodeData;
  ...
  return (
    <>
      ...
      >
        ...
        {timestamp && <div className="text-[8px] mt-0.5 opacity-40">{timestamp}</div>}
        {childCount > 1 && (
...
export const RiverTreeNode = memo(RiverTreeNodeComponent);
